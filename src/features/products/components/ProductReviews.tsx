...
function ProductReviews({ productId }: { productId: number }) {
    ...
    const { data: reviews, isLoading: isLoadingReviews } = useFetchProductRevviews(productId, { enabled: isReviewsOpen });
                    ...
                    <AccordionContent>
                        {isLoadingReviews ? (
                            ...
                            <ProductReviewsList>
                                {reviews?.map((review) => {
                                    return (
                                        <li key={review.id} className="flex flex-col gap-sm p-sm border rounded-md">
